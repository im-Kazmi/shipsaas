...
export const drizzleInstaller: Installer = ({
  ...
  const apiClient = usingServerActions ? "serveractions" : "hono";
  const payment = usingLemon ? "lemon" : usingStripe ? "stripe" : "";

  const schemaSrc = path.join(
    extrasDir,
    "src/server/db/schema-drizzle",
    auth
      ? `with-${auth}/with-${databaseProvider}${payment && "-" + payment}.ts`
