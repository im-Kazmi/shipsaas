...
// please run `prisma generate` to get types if you didnt before
...
export async function validateSessionToken(
  ...
  if (Date.now() >= sessionInDb.expiresAt.getTime()) {
    await db.session.delete({ where: { id: sessionInDb.id } });
    return { session: null, user: null };
  }
  ...
  if (
    ...
    const updatedSession = await db.session.update({
      ...
      data: { expiresAt: new Date(Date.now() + SESSION_MAX_DURATION_MS) },
    });
